...
public class Javascript {
	...
	public Javascript(HttpServletResponse response, String content,
			String charset) {
		this.response = response;
	 ...
	 * @throws IOException
	 */
	public void alert(String script) throws IOException {
	...
	public void closeOpenWindow() throws IOException {
		...
		strInfo = strInfo.append("<script language=\"JavaScript\">");
		strInfo = strInfo
				.append("window.close();window.opener.location.reload();");
		strInfo = strInfo.append("</script>");
		response.setContentType(content + ";charset=" + charset);
		try {
			response.getWriter().print(strInfo);
